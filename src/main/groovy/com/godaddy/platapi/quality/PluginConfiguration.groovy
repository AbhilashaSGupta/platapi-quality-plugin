...
class PluginConfiguration {
  ...
  String pmdVersion = '6.25.0'
  String spotbugsVersion = '4.0.6'

  /**
   * Automatically wire up the path of the supressions file.
   * This allows to ignore certain packages like generated files
   **/
  boolean useSuppressions = true;

  /**
   * Automatically register platapi gradle plugins, based on configured (affected) sources ({@link #sourceSets}).
   ...
   * <p>
   * When disabled, platapi gradle plugins must be registered manually. Only registered plugins will be configured
  ...
  /**
   * By default, cpd looks in all sources (cpd gradle plugin behaviour). When option enabled, platapi gradle plugin will
   * exclude all not configured source sets from cpd task sources. In case of multi-module build, where
   * cpd project declared in root project, all subprojects with platapi gradle plugin will exclude their sourceSets not
  ...
  /**
   * Direct sources to exclude from checks.
   * This is useful as last resort, when extension or package is not enough for filtering.
   * Use {@link org.gradle.api.Project#files(java.lang.Object)} or {@link org.gradle.api.Project#fileTree(java.lang.Object)}
